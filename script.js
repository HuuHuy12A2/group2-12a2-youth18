...
function drawLayer(cx, dArr, sArr, t, warm, rayCache){
    ...
    for(var i = 0; i < dArr.length; i++){
        var p = dArr[i];
        steer(p, t);
        /* Kiểm tra bụi trong ray */
        var lf = dustRayLight(p, rayCache);
        var op = p.op + Math.sin(t*0.7+p.ph)*0.04;
        op = cl(op, 0, 1);
        if(lf > 0.05){
            /* TRONG TIA NẮNG — vẽ hào quang ấm + core sáng */
            var gs = p.sz*10*(1+lf*1.5);
            cx.globalAlpha = op*lf*0.75;
            cx.drawImage(dustGlowTx, p.x-gs/2, p.y-gs/2, gs, gs);
            var cs = p.sz*5*(1+lf*0.5);
            cx.globalAlpha = Math.min(1, op*(1.2+lf*4.5));
            var cr = 255, cg = 235, cb = 150;
            var g = cx.createRadialGradient(p.x,p.y,0,p.x,p.y,Math.max(EPS,cs));
            g.addColorStop(0, 'rgba('+cr+','+cg+','+cb+','+cx.globalAlpha.toFixed(3)+')');
            g.addColorStop(0.4, 'rgba('+cr+','+cg+','+cb+','+(cx.globalAlpha*0.3).toFixed(3)+')');
            g.addColorStop(1, 'rgba('+cr+','+cg+','+cb+',0)');
            cx.beginPath(); cx.arc(p.x, p.y, Math.max(EPS, cs), 0, Math.PI*2);
            cx.fillStyle = g; cx.fill();
        }
        else {
            /* NGOÀI TIA NẮNG — vẽ bình thường, tối hơn */
            var realOp = op * 0.6;
            var cr2 = warm ? 255 : 212;
            var cg2 = warm ? 222 : 192;
            var cb2 = warm ? 165 : 148;
            var r = Math.max(EPS, p.sz);
            var g2 = cx.createRadialGradient(p.x,p.y,0,p.x,p.y,r);
            g2.addColorStop(0, 'rgba('+cr2+','+cg2+','+cb2+','+realOp.toFixed(3)+')');
            g2.addColorStop(0.45, 'rgba('+cr2+','+cg2+','+cb2+','+(realOp*0.32).toFixed(3)+')');
    ...
    cx.globalAlpha = 1;
    /* Sparkle — bay giống bụi, nháy sáng */
...
function loop(now){
    ...
    drawLayer(cxB, dustB, spkB, gTime, false, raysBCache);
    drawLayer(cxT, dustT, spkT, gTime, true, raysBCache.concat(raysTCache));
    tickFog(now);
